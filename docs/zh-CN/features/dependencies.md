...
可通过可点击的分类卡片、分类选择器和搜索框收窄表格。搜索范围包括库名、版本、导入模块、组件别名、Hook 入口点和 Hook 类型。表格展示依赖库、分类、可获取时的安装版本、导入模块名和运行时模块数量。
...
当同一种有序 Hook 存在多条注册时，工作区会提示涉及的类型并展示当前实际生效顺序；若未显式设置优先级，不应假定该顺序在不同环境中完全一致。
