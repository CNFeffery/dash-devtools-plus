...
The repository includes four runnable applications. Each calls `configure_devtools_plus` with the repository root, so the package can be exercised from a checkout without installing it into the environment first.
